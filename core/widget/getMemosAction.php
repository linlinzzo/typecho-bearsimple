...
$data = [];
        foreach($val['resourceList'] as $v){
            ...
            if(!$v['externalLink']){
            $data[] = '<img data-fancybox="image" data-caption="'.$v['filename'].'" '.lazyload('auto').'src="'.$temoptions['memos_Url'].'/o/r/'.$v['id'].'" data-memo-image>';  
            }
            else{
            $data[] = '<img data-fancybox="image" data-caption="'.$v['filename'].'" '.lazyload('auto').'src="'.$v['externalLink'].'" data-memo-image>';      
            }
            }
            //反之则当做超链接处理
            else{
